...
|  [`on(name: 'create', callback, ctx)`](on(''creat).on(''create'').md) |  | Called when a file is created. This is also called when the vault is first loaded for each existing file If you do not wish to receive create events on vault load, register your event handler inside [Workspace.onLayoutReady()](Workspace/onLayoutReady)<!-- -->. |
|  [`on(name: 'modify', callback, ctx)`](on(''modif).on(''modify'').md) |  | Called when a file is modified. |
|  [`on(name: 'delete', callback, ctx)`](on(''delet).on(''delete'').md) |  | Called when a file is deleted. |
|  [`on(name: 'rename', callback, ctx)`](on(''renam).on(''rename'').md) |  | Called when a file is renamed. |
