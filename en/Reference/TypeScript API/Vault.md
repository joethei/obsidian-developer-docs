...
|  [`on(name: 'create', callback, ctx)`](on__create__/on__create__) |  | Called when a file is created. This is also called when the vault is first loaded for each existing file If you do not wish to receive create events on vault load, register your event handler inside [Workspace.onLayoutReady()](Workspace/onLayoutReady)<!-- -->. |
|  [`on(name: 'modify', callback, ctx)`](on__modify__/on__modify__) |  | Called when a file is modified. |
|  [`on(name: 'delete', callback, ctx)`](on__delete__/on__delete__) |  | Called when a file is deleted. |
|  [`on(name: 'rename', callback, ctx)`](on__rename__/on__rename__) |  | Called when a file is renamed. |
