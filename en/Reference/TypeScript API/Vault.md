...
|  [`on(name: 'create', callback, ctx)`](Vault/on('na).md) |  | Called when a file is created. This is also called when the vault is first loaded for each existing file If you do not wish to receive create events on vault load, register your event handler inside [Workspace.onLayoutReady()](Workspace/onLayoutReady)<!-- -->. |
|  [`on(name: 'modify', callback, ctx)`](Vault/on('na).md) |  | Called when a file is modified. |
|  [`on(name: 'delete', callback, ctx)`](Vault/on('na).md) |  | Called when a file is deleted. |
|  [`on(name: 'rename', callback, ctx)`](Vault/on('na).md) |  | Called when a file is renamed. |
