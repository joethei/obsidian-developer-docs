...
|  [`on(name: 'editor-paste', callback, ctx)`]() |  | Triggered when the editor receives a paste event. Check for <code>evt.defaultPrevented</code> before attempting to handle this event, and return if it has been already handled. Use <code>evt.preventDefault()</code> to indicate that you've handled the event. |
|  [`on(name: 'editor-drop', callback, ctx)`]() |  | Triggered when the editor receives a drop event. Check for <code>evt.defaultPrevented</code> before attempting to handle this event, and return if it has been already handled. Use <code>evt.preventDefault()</code> to indicate that you've handled the event. |
...
|  [`on(name: 'resize', callback, ctx)`]() |  |  |
|  [`on(name: 'active-leaf-change', callback, ctx)`]() |  |  |
|  [`on(name: 'file-open', callback, ctx)`]() |  |  |
|  [`on(name: 'layout-change', callback, ctx)`]() |  |  |
|  [`on(name: 'window-open', callback, ctx)`]() |  |  |
|  [`on(name: 'window-close', callback, ctx)`]() |  |  |
