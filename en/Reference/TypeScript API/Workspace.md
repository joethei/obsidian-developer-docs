...
|  [`on(name: 'quick-preview', callback, ctx)`](Workspace/on('on(name:).md) |  |  |
|  [`on(name: 'files-menu', callback, ctx)`](Workspace/on('on(name:).md) |  | Triggered when the user opens the context menu with multiple files selected in the File Explorer. |
|  [`on(name: 'url-menu', callback, ctx)`](Workspace/on('on(name:).md) |  | Triggered when the user opens the context menu on an external URL. |
|  [`on(name: 'editor-menu', callback, ctx)`](Workspace/on('on(name:).md) |  | Triggered when the user opens the context menu on an editor. |
|  [`on(name: 'editor-change', callback, ctx)`](Workspace/on('on(name:).md) |  | Triggered when changes to an editor has been applied, either programmatically or from a user event. |
|  [`on(name: 'editor-paste', callback, ctx)`](Workspace/on('on(name:).md) |  | Triggered when the editor receives a paste event. Check for <code>evt.defaultPrevented</code> before attempting to handle this event, and return if it has been already handled. Use <code>evt.preventDefault()</code> to indicate that you've handled the event. |
|  [`on(name: 'editor-drop', callback, ctx)`](Workspace/on('on(name:).md) |  | Triggered when the editor receives a drop event. Check for <code>evt.defaultPrevented</code> before attempting to handle this event, and return if it has been already handled. Use <code>evt.preventDefault()</code> to indicate that you've handled the event. |
|  [`on(name: 'quit', callback, ctx)`](Workspace/on('on(name:).md) |  | Triggered when the app is about to quit. Not guaranteed to actually run. Perform some best effort cleanup here. |
|  [`on(name: 'resize', callback, ctx)`](Workspace/on('on(name:).md) |  |  |
|  [`on(name: 'active-leaf-change', callback, ctx)`](Workspace/on('on(name:).md) |  |  |
|  [`on(name: 'file-open', callback, ctx)`](Workspace/on('on(name:).md) |  |  |
|  [`on(name: 'layout-change', callback, ctx)`](Workspace/on('on(name:).md) |  |  |
|  [`on(name: 'window-open', callback, ctx)`](Workspace/on('on(name:).md) |  |  |
|  [`on(name: 'window-close', callback, ctx)`](Workspace/on('on(name:).md) |  |  |
|  [`on(name: 'css-change', callback, ctx)`](Workspace/on('on(name:).md) |  | Triggered when the CSS of the app has changed. |
|  [`on(name: 'file-menu', callback, ctx)`](Workspace/on('on(name:).md) |  | Triggered when the user opens the context menu on a file. |
